          ...
          if (stop) {
            ...
            fetch('/api/interview', {
              ...
              .then((data) => {
                ...
                try {
                  // Parse the API structure (e.g., Gemini/Google-like format)
                  text = JSON.parse(data)[0].parts[0].text;
                  console.log("Inner raw text:", text);
      
                  // Remove markdown if present
                  if (typeof text === 'string' && text.startsWith('```json')) {
                    text = text.slice(7, -3).trim();
                  }
      
                  // Extract actual JSON array using regex
                  const match = text.match(/\[\s*{[\s\S]*?}\s*\]/);
                  if (!match) throw new Error("No valid JSON array found in response");
      
                  const parsed = JSON.parse(match[0]); // Clean parse
                  setFeedback(parsed);
                  setStarted(false);
                  setMsgs([]);
                  setUserResponse('');
                } catch (e) {
                  console.error("Error parsing feedback JSON:", e, text);
                }
              })
              .catch((error) => console.error('Error:', error))
              .finally(() => {
            ...
            setConversation((prev) => [
              ...
              {
                role: 'user',
                parts: [{ text: `User Said: "${userResponse}". Now, ask the NEXT QUESTION.` }],
              },
            ]);
          }
        }
                        ...
                        <div className='flex flex-row my-4 align-center'>
                            ...
                            {!sending && <div className="flex self-center me-4">
                                <input
                                    ...
                                    id="conclude"
                                />
                                <label htmlFor="conclude">Conclude Interview (Check the box and give your final responce, to end the session and get feedback)</label>
                            </div>}
...

            {
                feedback && (
                    <div className="pb-4">
                        ...
                        <h4 className="text-lg font-semi-bold my-6">Feedback score is out of 5</h4>
